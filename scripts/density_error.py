...
if __name__ == "__main__":
    ...
    borehole_sites: Dict[str, Tuple[float, float]] = {"CI-0": (-66.403, -63.376),
                                                      "CI-22": (-66.588, -63.212),
    ...
    summer_2014 = [2014 + 11/12, 2015 + 2/12] # astrochronological summer of 2014 (December 2014 - February 2015)
    summer_2015 = [2015 + 11/12, 2016 + 2/12] # astrochronological summer of 2015 (December 2015 - February 2016)
    ...
    for site, (lat, lon) in borehole_sites.items():
        ...
        for physrho in physrho_values:
            ...
            summer_2014_mask = (results_dict["model_time_vector"] >= summer_2014[0]) & (results_dict["model_time_vector"] <= summer_2014[1])
            summer_2015_mask = (results_dict["model_time_vector"] >= summer_2015[0]) & (results_dict["model_time_vector"] <= summer_2015[1])
            ...
            for i in range(density_summer.shape[0]):
                density_summer_interp[i, :] = np.interp(in_situ_depth, results_dict["depth"].values, density_summer[i, :])

            # get rmse at each depth, averaging over time steps
            rmse = np.sqrt(np.mean((density_summer_interp - in_situ_density)**2, axis=0))
        ...
        plt.grid()
        plt.xlim(0, 45)
        plt.savefig(f"/home/speersm/luna/CPOM/speersm/CFM_data/cfm_figures/errors/{site}_density_rmse.png")
