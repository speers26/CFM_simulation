...
if __name__ == "__main__":
    ...
    lat_mask = (ref_data["LAT"] >= config["larsenC_box"]["lat_min"]) & (
        ref_data["LAT"] <= config["larsenC_box"]["lat_max"]
    )
    lon_mask = (ref_data["LON"] >= config["larsenC_box"]["lon_min"]) & (
        ref_data["LON"] <= config["larsenC_box"]["lon_max"]
    )
    combined_mask = lat_mask & lon_mask
