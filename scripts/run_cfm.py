...
df_daily = pd.read_csv(f"{config['CFM_data_path']}/cfm_input/MAR_{lat_val}_{lon_val}_{config['start_year']}_{config['end_year']}.csv", index_col=0, parse_dates=True)
...
variable_srho = False
if variable_srho:
    c['variable_srho'] = config["variable_srho"] # whether to use variable surface density
...
climateTS, StpsPerYr, depth_S1, depth_S2, grid_bottom, SEBfluxes = (
    ...
    spin_date_end = sde, melt=c['MELT'], desired_depth = None, SEB=c['SEB'], rho_bottom=config['rho_bottom']))
